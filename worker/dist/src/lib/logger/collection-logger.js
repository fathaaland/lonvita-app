 ...
 */
const changedFieldNames = (doc, previousDoc) => {
    if (!doc || !previousDoc || typeof doc !== 'object' || typeof previousDoc !== 'object')
        return [];
    const current = doc;
...
const logDelete = ({ doc, id, req, collection }) => {
    ...
    logger.info(`${slug}.delete`, {
        ...
        deletedTitle: doc?.title ?? doc?.name,
        ...baseContext(req, slug),
    });
    return doc;
};
 ...
 */
const logOperation = ({ operation, req, collection, result }) => {
    if (SKIPPED_OPERATIONS.has(operation))
        return result;
    const slug = collection.slug;
 ...
 */
export const withCrudLogging = (collection) => ({
    ...collection,
    hooks: {
        ...collection.hooks,
