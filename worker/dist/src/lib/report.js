...
function windowMetrics(events, regs, allEvents, profiles50Plus, windowStart, windowEnd, prevWindowStart, prevWindowEnd) {
    ...
    const wRegs = regs.filter((r) => r.status === "approved" &&
        wEventIds.has(r.event_id));
    // "Míra zapojení" and retention track who actually showed up, not who was merely approved.
    const participants = new Set(regs.filter((r) => r.attendance_status === "attended" && wEventIds.has(r.event_id)).map((r) => r.user_id));
    // Fill rate — jen akce s kapacitou
    const fills = [];
    for (const e of wEvents) {
        if (!e.capacity)
            continue;
        const cnt = regs.filter((r) => r.event_id === e.id && r.status === "approved").length;
    ...
    for (const e of allEvents) {
        ...
        if (prev === undefined || t < prev)
            firstEventByOrg.set(e.organizer_id, t);
    }
    let newOrganizers = 0;
    for (const oid of orgIds) {
        const first = firstEventByOrg.get(oid);
        if (first !== undefined && first >= ws && first < we)
            newOrganizers++;
    }
    // Zapojení 50+
    const participants50 = new Set(Array.from(participants).filter((uid) => profiles50Plus.has(uid)));
    const engagementRate50Plus = profiles50Plus.size > 0 ? participants50.size / profiles50Plus.size : 0;
    // Retence: z účastníků minulého okna kolik se vrátilo teď
    const pws = prevWindowStart.getTime();
    const pwe = prevWindowEnd.getTime();
    const prevEventIds = new Set(events
        .filter((e) => {
        const t = new Date(e.date_time).getTime();
        return t >= pws && t < pwe;
    })
        .map((e) => e.id));
    const prevParticipants = new Set(regs
        .filter((r) => r.attendance_status === "attended" && prevEventIds.has(r.event_id))
        .map((r) => r.user_id));
    let returning = 0;
    for (const uid of prevParticipants)
...
export function computeReportMetrics(events, regs, profiles) {
    const now = new Date();
    ...
    const profiles50Plus = new Set(profiles
        .filter((p) => p.date_of_birth && ageOn(p.date_of_birth, now) >= 50)
        .map((p) => p.id));
    const current = windowMetrics(events, regs, events, profiles50Plus, currentStart, currentEnd, previousStart, previousEnd);
    ...
    prevPrevStart.setDate(prevPrevStart.getDate() - 90);
    const previous = windowMetrics(events, regs, events, profiles50Plus, previousStart, previousEnd, prevPrevStart, prevPrevEnd);
    ...
    const prevPrevStart90 = new Date(prevPrevEnd90);
    prevPrevStart90.setDate(prevPrevStart90.getDate() - 90);
    const prevParticipantsForCurrent = new Set(regs
        .filter((r) => {
        ...
        .map((r) => r.user_id));
    const prevParticipantsForPrevious = new Set(regs
        .filter((r) => {
        if (r.attendance_status !== "attended")
            return false;
        const ev = events.find((e) => e.id === r.event_id);
        ...
        .map((r) => r.user_id));
    const dvCurrent = computeDatavitaWindow(events, regs, profiles50Plus, currentStart.getTime(), currentEnd.getTime(), prevParticipantsForCurrent);
    const dvPrevious = computeDatavitaWindow(events, regs, profiles50Plus, previousStart.getTime(), previousEnd.getTime(), prevParticipantsForPrevious);
    const delta = dvCurrent.score !== null && dvPrevious.score !== null ? dvCurrent.score - dvPrevious.score : 0;
    const trend = dvCurrent.score === null ? "nedostatek dat" : delta >= 3 ? "roste" : delta <= -3 ? "klesá" : "stabilní";
...
export function deltaPP(current, previous) {
    const diff = (current - previous) * 100;
    const sign = diff > 0 ? "+" : "";
    return `${sign}${diff.toFixed(1)} p. b.`;
}
export function deltaAbs(current, previous) {
    const diff = current - previous;
    const sign = diff > 0 ? "+" : "";
    return `${sign}${diff}`;
}
export function deltaPctChange(current, previous) {
    if (previous === 0)
        return current > 0 ? "nové" : "0 %";
    const diff = ((current - previous) / previous) * 100;
    const sign = diff > 0 ? "+" : "";
    return `${sign}${diff.toFixed(0)} %`;
}
export function buildReportRows(m) {
    const c = m.current;
    const p = m.previous;
    return [
        {
