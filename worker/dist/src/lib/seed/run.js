...
const DEV_ACCOUNTS = [
    ...
    { email: 'admin@admin.cz', password: 'admin1234', platformRole: 'user' },
    { email: 'ucastnik@ucastnik.cz', password: 'ucastnik1234', platformRole: 'user' },
];
...
async function bootstrapProductionSuperadmin(payload) {
    ...
    if (password.length < SUPERADMIN_PASSWORD_MIN_LENGTH) {
        payload.logger.warn(`SEED_SUPERADMIN_PASSWORD must be at least ${SUPERADMIN_PASSWORD_MIN_LENGTH} characters — skipping superadmin bootstrap.`);
        return;
    }
...
const MUNICIPALITIES = [
    {
        name: 'Nové Veselí',
        description: 'Nové Veselí na Žďársk a okolí a na Vysočině a Česku',
        lat: 49.5198752,
        lng: 15.9084979,
    },
...
const DEMO_EVENTS = [
    {
        ...
        description: 'Posedíme u kávy a domácího dortu a popovídáme si o tom, co se v obci chystá. Přijďte klidně i sami — rádi vás seznámíme s ostatními.',
        municipality: 'Nové Veselí',
    ...
    },
    {
        ...
        description: 'Nenáročná asi čtyřkilometrová procházka po rovině kolem rybníka. Tempo přizpůsobíme všem, po cestě jsou lavičky na odpočinek.',
        municipality: 'Nové Veselí',
        organizer: 'admin@admin.cz',
    ...
    },
    {
        ...
        description: 'Společně si přečteme povídky českých autorů a u čaje si o nich popovídáme. Knihy zapůjčíme, stačí přijít.',
        municipality: 'Nové Veselí',
        organizer: 'admin@admin.cz',
    ...
    },
    {
        ...
        description: 'Přátelský turnaj pro začátečníky i zkušené hráče. Koule zapůjčíme, hraje se ve dvojicích, které losujeme na místě.',
        municipality: 'Nové Veselí',
...
async function seedUser(payload, input) {
...
async function findUserId(payload, email) {
...
async function ensureMunicipality(payload, input) {
    const existing = await payload.find({
...
const localDateTime = (inDays, [hours, minutes]) => {
    const date = new Date();
    date.setDate(date.getDate() + inDays);
    date.setHours(hours, minutes, 0, 0);
    return date.toISOString();
};
async function seedDemoData(payload, categoryIds) {
...
export async function runSeed(payload, { includeDemoData = true } = {}) {
    ...
    if (includeDemoData && shouldSeedDemoData()) {
        for (const account of DEV_ACCOUNTS) {
            ...
            if (isProduction() && account.platformRole === 'admin')
                continue;
