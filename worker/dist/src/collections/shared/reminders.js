...
const relId = (value) => (typeof value === 'object' ? value.id : value);
const attendanceJobId = (eventId) => `attendance-reminder-${eventId}`;
const participantJobId = (registrationId) => `reminder-${registrationId}`;
...
export async function scheduleAttendanceReminder(payload, event) {
...
export async function scheduleParticipantReminder(payload, registrationId, userId, event) {
    ...
    if (!user?.email || !(profiles.docs[0]?.notifyEmail ?? true))
        return;
...
export async function rescheduleEventReminders(payload, event) {
...
export async function cancelEventReminders(payload, eventId) {
