 ...
 */
export const guardCancellationWindow = ({ data, originalDoc, operation }) => {
    if (operation !== 'update' || !data || !originalDoc)
        return data;
    const cancelling = (data.deletedAt && !originalDoc.deletedAt) || (data.status === 'cancelled' && originalDoc.status !== 'cancelled');
    if (!cancelling)
        return data;
    if (!canCancelEvent(originalDoc.dateTime)) {
        throw new APIError(`Akci lze zrušit nejpozději ${EVENT_CANCELLATION_CUTOFF_HOURS} hodiny před jejím začátkem.`, 400);
    }
    return data;
};
