...
const preventDuplicateName = async ({ data, req, operation, originalDoc }) => {
    if (!data?.name)
        return data;
    ...
    const duplicate = candidates.docs.some((doc) => doc.name.trim().toLowerCase() === name.toLowerCase() && doc.id !== originalDoc?.id);
    if (duplicate) {
...
const grantRoleForAdminUser = async ({ doc, previousDoc, req, context }) => {
    if (context?.skipAdminUserSync)
        return doc;
...
const canUpdateOwnMunicipality = async ({ req }) => {
...
export const Municipalities = {
    ...
    fields: [
        ...
        },
        {
            ...
            required: true,
            admin: {
                description: 'Brief §4 "poloha je mandatory field" — plots this obec on the municipality-switcher map (brief §4/§7 "mapka jako v Projects/eduard-app").',
            },
        },
        {
            name: 'lng',
        ...
        },
        {
            ...
            admin: {
                description: 'The user who administers this municipality. Kept in sync with "municipality_admin" user-roles (the most recently granted one).',
                position: 'sidebar',
            },
        },
        {
            ...
            defaultValue: 15,
            admin: {
                description: 'Maximální vzdálenost (km) mezi středem obce (lat/lng výše) a místem konání akce. Akce mimo tento okruh nejde založit ani upravit — brání přiřazení akce z jiného města k této obci.',
            },
        },
        {
            ...
            admin: {
                description: 'Brief §3 "Pravidla pro vznik akcí" — controls who may create events for this municipality (see Events.access.create). A signed-out visitor is always read-only regardless of this setting. The former "anyone can create" option was removed for security (task 5) — an obec admin must never be able to create events outside their own obec, which an open-to-anyone mode elsewhere would have allowed.',
            },
        },
    ],
    hooks: {
