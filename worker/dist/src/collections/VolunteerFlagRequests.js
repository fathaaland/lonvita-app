...
export const VolunteerFlagRequests = {
    ...
    hooks: {
        beforeValidate: [
            async ({ data, req, operation }) => {
                if (!data?.event || !data?.requestedBy)
                    return data;
                if (operation !== 'create')
                    return data;
                ...
                const coOrganizerIds = (event.coOrganizers ?? []).map((c) => typeof c === 'object' ? c.id : c);
                const isOrganizerOfEvent = String(organizerId) === String(data.requestedBy) ||
                    coOrganizerIds.some((id) => String(id) === String(data.requestedBy));
