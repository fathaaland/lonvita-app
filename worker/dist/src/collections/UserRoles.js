...
async function syncMunicipalityAdminUser(req, municipalityId) {
...
export const UserRoles = {
    ...
    fields: [
        ...
        },
        {
            ...
            admin: {
                description: 'Enum matches ERD §0.2 pilot roles, plus "organizer" (brief §4). A user can hold the same role in several municipalities at once — organizers and volunteers aren\'t tied to one town (brief §8 "Působení jednoho člověka napříč víc obcemi"). "prescriber" is a reserved slot for the intervention layer — not wired to any workflow yet (see brief §B).',
            },
        },
    ],
    hooks: {
        beforeValidate: [
            async ({ data, req, operation, originalDoc }) => {
                if (!data?.user || !data?.role || !data?.municipality)
                    return data;
        ...
        afterChange: [
            async ({ doc, previousDoc, req }) => {
                ...
                if (doc.role === 'municipality_admin')
                    municipalityIds.add(relId(doc.municipality));
