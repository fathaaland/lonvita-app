...
const setGeneratedPasswordIfMissing = ({ data, operation }) => {
    if (operation !== 'create')
        return data;
    const userData = (data ?? {});
 ...
 */
const lockPlatformRole = async ({ args, operation, req }) => {
    if (operation !== 'update')
        return args;
    const { data, id, overrideAccess } = args;
    ...
    if (overrideAccess !== false || data?.role === undefined || id === undefined)
        return args;
...
const cleanupUserRelations = async ({ id, req }) => {
    const { payload } = req;
    ...
    if (organized.totalDocs > 0) {
        throw new APIError(`Uživatel je pořadatelem ${organized.totalDocs} akcí. Nejdřív je zrušte nebo předejte jinému pořadateli, potom půjde účet smazat.`, 400);
    }
...
export const Users = {
    ...
    fields: [
        // email is added automatically by Payload auth
        {
            ...
            admin: {
                position: 'sidebar',
                description: 'Platform-level role — controls Payload admin access, not community roles. Not editable through the app: grant it from a seed/script (Local API) only.',
            },
        },
    ],
    hooks: {
